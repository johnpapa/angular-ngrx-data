...
describe('EntityChangeTrackerBase', () => {
  ...
  describe('#commitAll', () => {
    it('should clear all tracked changes', () => {
      ...
      expect(Object.keys(collection.changeState).length).toBe(3, 'tracking 3 entities');

      collection = tracker.commitAll(collection);
      expect(Object.keys(collection.changeState).length).toBe(0, 'tracking zero entities');
    });
  });
  ...
  describe('#commitMany', () => {
    it('should clear current tracking of the given entities', () => {
      ...
      expect(collection.changeState[addedEntity.id]).toBeUndefined('no changes tracked for added entity');
      expect(collection.changeState[updatedEntity.id]).toBeUndefined('no changes tracked for updated entity');
      expect(collection.changeState[deletedEntity.id]).toBeDefined('still tracking deleted entity');
    });
  });
  ...
  describe('#trackAddOne', () => {
    it('should return a new collection with tracked new entity', () => {
      ...
      expect(change.originalValue).toBeUndefined('no original value for a new entity');
    });
    ...
    it('should leave added entity tracked as added when entity is updated', () => {
      ...
      expectChangeType(change, ChangeType.Added);
      expect(change.originalValue).toBeUndefined('still no original value for added entity');
    });
    ...
    it('should return the same collection if MergeStrategy.IgnoreChanges', () => {
      const addedEntity = { id: 42, name: 'Ted', power: 'Chatty' };
      const collection = tracker.trackAddOne(addedEntity, origCollection, MergeStrategy.IgnoreChanges);
  ...
  describe('#trackAddMany', () => {
    const newEntities = [{ id: 42, name: 'Ted', power: 'Chatty' }, { id: 84, name: 'Sally', power: 'Laughter' }];
    ...
    it('should return a new collection with tracked new entities', () => {
      ...
      trackKeys.forEach((key, ix) => {
        ...
        expectChangeType(change, ChangeType.Added, `tracking ${key} as a new entity`);
        expect(change.originalValue).toBeUndefined(`no original value for new entity ${key}`);
      });
    });
  ...
  describe('#trackDeleteOne', () => {
    it('should return a new collection with tracked "deleted" entity', () => {
      ...
      expect(change.originalValue).toBe(existingEntity, 'originalValue is the existing entity');
    });
    ...
    it('should switch an updated entity to a deleted entity when it is removed', () => {
      const existingEntity = getFirstExistingEntity();
      const updatedEntity = toUpdate({ ...existingEntity, name: 'test update' });

      let collection = tracker.trackUpdateOne(toUpdate(updatedEntity), origCollection);
    ...
    it('should leave deleted entity tracked as deleted when try to update', () => {
      ...
      expectChangeType(change, ChangeType.Deleted);
      expect(change.originalValue).toEqual(existingEntity, 'still tracking original value');
    });
    ...
    it('should return same collection if MergeStrategy.IgnoreChanges', () => {
      const existingEntity = getFirstExistingEntity();
      const collection = tracker.trackDeleteOne(existingEntity.id, origCollection, MergeStrategy.IgnoreChanges);
      expect(collection).toBe(origCollection);
      const change = collection.changeState[existingEntity.id];
      expect(change).toBeUndefined('not tracking the entity');
    });
  ...
  describe('#trackDeleteMany', () => {
    it('should return a new collection with tracked "deleted" entities', () => {
      ...
      existingEntities.forEach((entity, ix) => {
        ...
        expect(change.originalValue).toBe(existingEntities[ix], `entity #${ix} originalValue`);
      });
    });
  ...
  describe('#trackUpdateOne', () => {
    ...
    it('should return same collection if MergeStrategy.IgnoreChanges', () => {
      const existingEntity = getFirstExistingEntity();
      const updatedEntity = toUpdate({ ...existingEntity, name: 'test update' });
      const collection = tracker.trackUpdateOne(updatedEntity, origCollection, MergeStrategy.IgnoreChanges);
      expect(collection).toBe(origCollection);
      const change = collection.changeState[existingEntity.id];
      expect(change).toBeUndefined('not tracking the entity');
    });
  ...
  describe('#trackUpdateMany', () => {
    it('should return a new collection with tracked updated entities', () => {
      const existingEntities = getSomeExistingEntities(2);
      const updateEntities = existingEntities.map(e => toUpdate({ ...e, name: e.name + ' updated' }));
    ...
    it('should return same collection if called with entities whose keys are not found', () => {
      const updateEntities = [toUpdate({ id: '1234', name: 'Mr. 404' }), toUpdate({ id: 456, name: 'Ms. 404' })];
      const collection = tracker.trackUpdateMany(updateEntities, origCollection);
      expect(collection).toBe(origCollection);
    });
  });
  ...
  describe('#trackUpsertOne', () => {
    it('should return a new collection with tracked added entity', () => {
      ...
      expect(change).toBeDefined('tracking the entity');
      expectChangeType(change, ChangeType.Added);
      expect(change.originalValue).toBeUndefined('no originalValue for added entity');
    });
    ...
    it('should not change orig value of updated entity that is updated again', () => {
      ...
      expect(change).toBeDefined('is still tracked as an updated entity');
      expectChangeType(change, ChangeType.Updated, 'still updated after attempted add');
      expect(change.originalValue).toEqual(existingEntity, 'still tracking original value');
    });
    ...
    it('should return same collection if MergeStrategy.IgnoreChanges', () => {
      ...
      const collection = tracker.trackUpsertOne(updatedEntity, origCollection, MergeStrategy.IgnoreChanges);
      expect(collection).toBe(origCollection);
      const change = collection.changeState[existingEntity.id];
      expect(change).toBeUndefined('not tracking the entity');
    });
  ...
  describe('#trackUpsertMany', () => {
    it('should return a new collection with tracked upserted entities', () => {
      ...
      const updatedEntities = exitingEntities.map(e => ({ ...e, name: e.name + 'test' }));
      const upsertEntities = updatedEntities.concat(addedEntity);
      const collection = tracker.trackUpsertMany(upsertEntities, origCollection);
      expect(collection).not.toBe(origCollection);
      updatedEntities.forEach((entity, ix) => {
        ...
        expectChangeType(change, ix === 2 ? ChangeType.Added : ChangeType.Updated, `entity #${ix}`);
        if (change.changeType === ChangeType.Updated) {
          expect(change.originalValue).toBe(exitingEntities[ix], `entity #${ix} originalValue`);
        } else {
          expect(change.originalValue).toBeUndefined(`no originalValue for added entity #${ix}`);
        }
      });
    });
  ...
  describe('#undoAll', () => {
    ...
    it('should restore the collection to the pre-change state', () => {
      // tslint:disable-next-line:prefer-const
      let { collection, addedEntity, deletedEntity, preUpdatedEntity, updatedEntity } = createTestTrackedEntities();

      // Before undo
      expect(collection.entities[addedEntity.id]).toBeDefined('added entity should be present');
      expect(collection.entities[deletedEntity.id]).toBeUndefined('deleted entity should be missing');
      expect(updatedEntity.name).not.toEqual(preUpdatedEntity.name, 'updated entity should be changed');

      collection = tracker.undoAll(collection);

      // After undo
      expect(collection.entities[addedEntity.id]).toBeUndefined('added entity should be removed');
      expect(collection.entities[deletedEntity.id]).toBeDefined('deleted entity should be restored');
      const revertedUpdate = collection.entities[updatedEntity.id];
      expect(revertedUpdate.name).toEqual(preUpdatedEntity.name, 'updated entity should be restored');
    });
  });
  ...
  describe('#undoOne', () => {
    it('should restore the collection to the pre-change state for the given entity', () => {
      ...
      expect(collection.entities[deletedEntity.id]).toBeDefined('deleted entity should be restored');
      expect(collection.entities[addedEntity.id]).toBeDefined('added entity should still be present');
      expect(updatedEntity.name).not.toEqual(preUpdatedEntity.name, 'updated entity should be changed');
    });
  ...
  describe('#undoMany', () => {
    it('should restore the collection to the pre-change state for the given entities', () => {
      ...
      collection = tracker.undoMany([addedEntity, deletedEntity, updatedEntity], collection);
      expect(collection.entities[addedEntity.id]).toBeUndefined('added entity should be removed');
      expect(collection.entities[deletedEntity.id]).toBeDefined('deleted entity should be restored');
      const revertedUpdate = collection.entities[updatedEntity.id];
      expect(revertedUpdate.name).toEqual(preUpdatedEntity.name, 'updated entity should be restored');
  ...
  function createTestTrackedEntities() {
    ...
    return { collection, addedEntity, deletedEntity, preUpdatedEntity, updatedEntity };
  }
